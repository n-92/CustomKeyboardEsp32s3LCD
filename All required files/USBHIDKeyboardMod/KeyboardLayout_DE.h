...
extern const char16_t KeyboardLayout_de_DE_special_characters[AMOUNT_OF_SPECIAL_CHARS] PROGMEM =
	{
		u'^',
		u'ß',
		u'´',
...
class KeyboardLayoutDE : public KeyboardLayout
{
public:
	KeyboardLayoutDE()
	{
	...
	uint16_t getKeycode(char16_t key)
	{
		...
		else
		{
			if (key < 128)
			{
				return KeyboardLayout_de_DE[key];
			}
	...
	uint16_t isSpecialKey(char16_t key)
	{
		...
		for (int i = 0; i < AMOUNT_OF_SPECIAL_CHARS; i++)
		{
			...
			Serial.println((char16_t)KeyboardLayout_de_DE_special_characters[i]);
			
			if (key == KeyboardLayout_de_DE_special_characters[i])
			{
				Serial.println("Compare keys");
				Serial.println(key);
				Serial.println((char16_t)KeyboardLayout_de_DE_special_characters[i]);
				Serial.println((char)key);
				Serial.println((char)KeyboardLayout_de_DE_special_characters[i]);
				
				
				keycode = KeyboardLayout_de_DE_special_keycodes[i];
